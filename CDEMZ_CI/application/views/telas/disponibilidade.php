<?php
defined('BASEPATH') or exit('No direct script access allowed');
?>
<!-- Available App  Start-->
<div class="available-app-area">
    <div class="container">
        <div class="row d-flex justify-content-between">
            <div class="col-xl-6 col-lg-6">
                <div class="app-caption">
                    <div class="section-tittle section-tittle3">
                        <h2>
                            Nossa aplicação está atualmente disponível para Android.
                        </h2>
                        <p>
                            Em breve estarão disponíveis as versões para IOS (Iphone) e
                            Web.
                        </p>
                        <div class="app-btn">
                            <!-- <a href="#" class="app-btn1"
                      ><img src="assets/img/shape/app_btn1.png" alt=""
                    /></a> -->
                            <a href="https://play.google.com/store/apps/details?id=mz.co.chillstudio.codigo_de_estrada_mz" class="app-btn2"><img src="<?php echo base_url('assets/img/shape/app_btn2.png') ?>" alt="" /></a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-6 col-lg-6">
                <div class="app-img">
                    <img src="<?php echo base_url('assets/img/shape/available-app.png') ?>" alt="" />
                </div>
            </div>
        </div>
    </div>
    <!-- Shape -->
    <div class="app-shape">
        <img src="<?php echo base_url('assets/img/shape/app-shape-top.png') ?>" alt="" class="app-shape-top heartbeat d-none d-lg-block" />
        <img src="<?php echo base_url('assets/img/shape/app-shape-left.png') ?>" alt="" class="app-shape-left d-none d-xl-block" />
        <!-- <img src="assets/img/shape/app-shape-right.png" alt="" class="app-shape-right bounce-animate "> -->
    </div>
</div>
<!-- Available App End-->